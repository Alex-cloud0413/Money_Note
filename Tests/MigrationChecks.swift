...
struct MigrationChecks {
    @MainActor static func main() throws {
        ...
        precondition(accounts[0].name == "测试账户" && accounts[0].initialBalance == 500)
        ...
        precondition(budgets[0].monthKey == SubscriptionEngine.monthKey(.now))
        precondition(subs[0].ledgerKey == "life" && subs[0].account?.name == "测试账户")
        print("PASS: real 1.0 to 1.3 migration preserves transactions and legacy relationships while localizing the old budget")

        let work = TxRecord(amount: 200, type: .expense, categoryName: "餐饮", categoryIcon: "", subcategoryName: "早餐", note: "事业测试")
        work.ledgerKey = "work"; context.insert(work)
        ...
        let workBudget = BudgetModel(amount: 800, ledgerKey: "work")
        workBudget.monthKey = SubscriptionEngine.monthKey(.now)
        context.insert(workBudget)
        ...
        precondition(generated.count == 1 && generated[0].ledgerKey == "work" && generated[0].account == nil)
        try SubscriptionEngine.sync(context)
